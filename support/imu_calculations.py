...
def calculate_imu_orientation(
    df, filter_size=51, filter_order=1, zero_drift=50, dt=0.01
):
    """
    ...
    _imu_df = _imu_df.with_columns(
        [pl.col("gx") - val_x, pl.col("gy") - val_y, pl.col("gz") - val_z]
    )
...
def calculate_imu_velocity(
    df, filter_size=None, filter_order=None, zero_drift=50, dt=0.01
):
    """
    df: the dataframe containing the imu data
    filter_size: the size of the filter window (should be odd)
    ...
    # subtracting the zero drift
    _imu_df = _imu_df.with_columns(
        [pl.col("ax") - val_x, pl.col("ay") - val_y, pl.col("az") - val_z]
    )
...
def calculate_imu_displacement(
    df, filter_size=None, filter_order=None, zero_drift=0, dt=0.01
):
    """
    df: the dataframe containing the imu data
    filter_size: the size of the filter window (should be odd)
    ...
    for i in range(len(_imu_df)):
        if i == 0:
            ...
            my_dict["disp_x"].append(
                my_dict["disp_x"][i - 1] + (_imu_df["vel_x"][i]) * dt
            )
            my_dict["disp_y"].append(
                my_dict["disp_y"][i - 1] + (_imu_df["vel_y"][i]) * dt
            )
            my_dict["disp_z"].append(
                my_dict["disp_z"][i - 1] + (_imu_df["vel_z"][i]) * dt
            )
