...
def _get_model_and_info(input_model_path: Path):
    ...
    print(classes_masks_out)
    boxes_out = model_output_shape.dim[2].dim_value
    assert classes_masks_out == 17
...
def _update_model(
    model: onnx.ModelProto, output_model_path: Path, pipeline: PrePostProcessor
):
    """
...
def _add_pre_post_processing_to_rgb_input(
    input_model_path: Path, output_model_path: Path, input_shape: List[Union[int, str]]
):
    """
    ...
    if input_shape[0] == 3:
        ...
        raise ValueError(
            "Invalid input shape. Either first or last dimension must be 3."
        )
    ...
    if layout == "CHW":
        ...
        pre_processing_steps = [
            ChannelsLastToChannelsFirst(name="RGBImageCHW")
        ]  # HWC to CHW
    ...
    pre_processing_steps += [
        ...
        LetterBox(
            target_shape=(h_in, w_in), layout="CHW"
    ...
    post_processing_steps = [
        Squeeze(
            [0]
        ),  # - Squeeze to remove batch dimension from [batch, 56, 8200] output
        ...
        Split(num_outputs=3, axis=1, splits=[4, 1, 12]),
        ...
        SelectBestBoundingBoxesByNMS(
            iou_threshold=0.7, score_threshold=0.25, has_mask_data=True
        ),
...
def _add_pre_post_processing_to_image_input(
    input_model_path: Path, output_model_path: Path, output_image_format: Optional[str]
):
    """
    ...
    if output_image_format:
        post_processing_steps += [
            ...
            (
                DrawBoundingBoxes(
                    mode="CENTER_XYWH", num_classes=1, colour_by_classes=True
                ),
                [
                    utils.IoMapEntry("BGRImageHWC", producer_idx=0, consumer_idx=0),
                    utils.IoMapEntry(
                        "SplitScaledBoxesAndKeypoints", producer_idx=0, consumer_idx=1
                    ),
                ],
            ),
            # Encode to jpg/png
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test_image",
        type=Path,
        default="data/stormtroopers.jpg",
    ...
    if args.output_image and args.input == "rgb":
        raise argparse.ArgumentError(
            args.output_image,
            "output_image argument can only be used if input is 'image'",
        )

    if args.input_shape and len(args.input_shape) != 3:
        raise argparse.ArgumentError(
            args.input_shape, "Shape of RGB input must have 3 dimensions."
        )
    ...
    if args.input == "rgb":
        ...
        _add_pre_post_processing_to_rgb_input(
            args.onnx_model_path, updated_model_path, args.input_shape
        )
        ...
        _add_pre_post_processing_to_image_input(
            args.onnx_model_path, updated_model_path, args.output_image
        )
        rgb_layout = None
