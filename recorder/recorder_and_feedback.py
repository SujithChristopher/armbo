...
rA_m1 = np.array([0.02, 0.0, -0.115]).reshape(3, 1)
...
while True:
    ...
    if ret:
        ...
        rotation_vectors, translation_vectors, _ = aruco.estimatePoseSingleMarkers(
            corners, 0.04, _webcam_cam_mat, _webcam_dist
        )
        ...
        try:
            ...
            _frame = cv2.putText(
                _frame,
                f"p1 C x :{round(translation_vectors[0][0][0], 3)}",
                (5, 20),
                font,
                fontScale,
                color,
            ...
            _frame = cv2.putText(
                _frame,
                f"p1 C y :{round(translation_vectors[0][0][1], 3)}",
                (5, 45),
                font,
                fontScale,
                color,
            ...
            _frame = cv2.putText(
                _frame,
                f"p1 C z :{round(translation_vectors[0][0][2], 3)}",
                (5, 70),
                font,
                fontScale,
                color,
            ...
            pb_C = (
                _ar_lframe_rot.T
                @ (translation_vectors[0][0].reshape(3, 1) - _ar_lframe_org)
            ).T[0]
            ...
            _frame = cv2.putText(
                _frame,
                f"Pa C x :{round(for_display[0], 3)}",
                (5, 280),
                font,
                fontScale,
                color,
            ...
            _frame = cv2.putText(
                _frame,
                f"Pa C y :{round(for_display[1], 3)}",
                (5, 310),
                font,
                fontScale,
                color,
            ...
            _frame = cv2.putText(
                _frame,
                f"Pa C z :{round(for_display[2], 3)}",
                (5, 340),
                font,
                fontScale,
                color,
